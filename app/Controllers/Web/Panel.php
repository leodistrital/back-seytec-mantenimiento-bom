...
use App\Models\DetagendaparticioantesApiModel;
...
class Panel extends MyApiRest
{
    ...
    public function index()
    {
        ...
        $array = ['finacreditacion', 'val_tar', 'usu_reg', 'tok_reg', 'descuento', 'formPart'];
        ...
        return view('panelcuenta', ['contenido' => $data[0], 'seccion' => $seccion, 'finacreditacion' => $datauser[0]['finacreditacion'], 'activarpago' => $activarpago, 'veracreditacion' => $veracreditacion, 'random' => $random, 'datauserpago' => $datauser[0], 'esprensa' => $esprensa, 'codigoagenda' => $iduser]);
        // return '';
    }
    ...
    public function guardarformacreditacion($paso)
    {
        ...
        if ($paso == 1) {
            $data = [
                ...
                'ciudadresidencia' => (empty($request->getVar('ciudadresidencia'))) ? 'BOGOTÁ. D.C.' : $request->getVar('ciudadresidencia'),
                ...
                'depresidencia' => $request->getVar('departamentosselector'),
                'grupoetniaotro' => $request->getVar('otrogrupoetnicotext'),
                'divpaso1' => 1
            ];
        }
        ...
        if ($paso == 4) {
            ...
            $rutdocumento = $this->uploadfile($ruta, 'rut');
    ...
    public function sendinblueenvio($correo, $nombre, $operacion, $opt = '')
    {
        if ($operacion == 'recibodaacreditacion') {
            $toName = $nombre;
            $toEmail = $correo;
            $subject = 'Confirmación solicitud de acreditación BAM 2024 / Confirmation of accreditation application BAM 2024';
        }
                    ...
                    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <h4>
                                <p style="margin: 80px 0px 0px; letter-spacing: 1px; font-size: 16px;">Yours sincerely , <br />BAM Team</p>
                                </h4>
                            </td>
                            <td width="80"></td>
                        </tr>
    ...
    public function agenda1111()
    {
        ...
        print_r($datauser[0]['finacreditacion']);
        ...
        return view('panelcuenta', ['contenido' => $data[0], 'seccion' => $seccion, 'finacreditacion' => 0, 'esprensa' => '', 'random' => $random, 'datauserpago' => $datauser[0]]);
        // echo "leo";
        // return '';
    }


    public function agendainterna($id)
    {
        $seccion = 'agenda';
        $activarpago = false;
        $veracreditacion = true;
        //3cec0f7570167930b34213d66d631e0a
        // exit();
        $iduser = $id;
        // $tokenconfirmation = $this->generarToken($iduser);  //genera token para pago
        $usuarios = new Acr_registoWebModel();
        $array = ['finacreditacion', 'usu_reg', 'tok_reg', 'cod_reg'];
        $datauser = $usuarios->select($array)->where('usu_reg', $iduser)->find();
        //echo $usuarios->getLastQuery();

        $codigoUsuario = $datauser[0]['cod_reg'];

        $fechasArray = ['2024-07-08', '2024-07-09', '2024-07-10', '2024-07-11', '2024-07-12'];
        $solodiaArray = ['08', '09', '10', '11', '12'];
        $diasArray = [
            'Lunes',
            'Martes',
            'Miércoles',
        ...
        foreach ($fechasArray as $key => $value) {
            // echo $value;
            $calendario[$key]['fecha'] = $value;
            $calendario[$key]['nombredia'] = $diasArray[$key];
            $calendario[$key]['solodia'] = $solodiaArray[$key];
            $calendario[$key]['citas'] = $this->buscarCitaFecha($value, $codigoUsuario);
        ...
        return view('panelcuenta', ['contenido' => $data[0], 'seccion' => $seccion, 'finacreditacion' => 0, 'esprensa' => '', 'datauserpago' => $datauser[0], 'calendario' => $calendario]);
        // echo "leo";
        // return '';
    }


    public function buscarCitaFecha($fecha, $codigoUsuario)
    {
        $agenda = new DetagendaparticioantesApiModel();
        $whereArray = ['cod_acr_dap' => $codigoUsuario, 'fec_age' => $fecha];
        $array = ['cod_age_dap', 'cod_acr_dap', 'fec_age', 'hor_ini_age', 'hor_fin_age', 'min_fin_age', 'min_ini_age', 'cod_lug_age', 'cod_ubi_age', 'not_age', 'tit_age', 'int_ini_age', 'int_fin_age', 'nom_ubi', 'nom_lug', "concat(LPAD(hor_ini_age,2,'0'),':',LPAD(min_ini_age,2,'0'), ' - ',LPAD(hor_fin_age,2,'0'), ':',LPAD(min_fin_age,2,'0') ) as horario", 'nom_tag as tipo', 'col_tag as color'];

        $dataagenda = $agenda->select($array)->join('agendamiento', 'cod_age=cod_age_dap')->join('agendalugares', 'cod_lug=cod_lug_age')->join('agendaubicacion', 'cod_ubi=cod_ubi_age', 'left')->join('agendatipo', 'tip_age=cod_tag')->where($whereArray)->groupBy('cod_age_dap')->orderBy('hor_ini_age, min_ini_age, int_ini_age')->find();

        //   echo $agenda->getLastQuery();
        foreach ($dataagenda as $key => $item) {
            // print_r($item['cod_age_dap']);
            $dataagenda[$key]['participantes'] = $this->buscarParticipantes($item['cod_age_dap']);
            // print_r($dataagenda[$key]);
            // echo "<br>";
        }
        return $dataagenda;
    ...
    public function buscarParticipantes($codigoAgenda)
    {
        $agenda = new DetagendaparticioantesApiModel();
        $whereArray = ['cod_age_dap' => $codigoAgenda];
        $array = ['cod_age_dap', 'cod_acr_dap', "concat(nom_reg,' ',ape_reg) as nombre"];
        $participante = $agenda->select($array)->join('acr_registo', 'cod_reg=cod_acr_dap')->where($whereArray)->limit(5)->find();
        // echo $agenda->getLastQuery();
        return $participante;
    }
    ...
    public function agendainternatest($id)
    {
        $seccion = 'agenda';
        $activarpago = false;
        $veracreditacion = true;
    ...
    public function agenda1($id)
    {
        ...
        // print_r($calendario[1]['citas']);
        $jsonArray = json_encode($calendario);
        ...
        $seccion = 'agendapublica';

        return view('panelcuenta', ['contenido' => $data[0], 'seccion' => $seccion, 'finacreditacion' => 0, 'esprensa' => '', 'datauserpago' => $datauser[0], 'calendario' => $calendario, 'jsonArray' => $jsonArray]);
        // echo "leo";
        // return '';
    }
    ...
    public function agenda2($id)
    {
        ...
        $seccion = 'agendapublica2';

        // var_dump($calendario[4]);
