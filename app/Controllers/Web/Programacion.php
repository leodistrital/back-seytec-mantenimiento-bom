...
class Programacion extends  MyApiRest
{
    ...
    public function programacion($edicion, $evento)
    {
        ...
        $array = ['cod_ave', 'tit_ave', 'des_ave', 'fec_ave', 'hor_ave', 'not_ave', 'pre_ave', 'vir_ave', 'url_ave', 'lug_ave', 'img_ave', "DATE_FORMAT(fec_ave,'%d') AS fecha", "concat(DAYNAME(fec_ave), ' ', MONTHNAME(fec_ave)   )  as dia", " DATE_FORMAT(hor_ave, '%l:%i %p ') as hora", 'ali_ave'];
        
        if (get_cookie('bamidioma') == "ENG") 
        $array = ['cod_ave', 'tit_ave_ing as tit_ave', 'des_ave_ing as des_ave', 'fec_ave', 'hor_ave', 'nor_ave_ing as not_ave', 'pre_ave_ing as pre_ave', 'vir_ave', 'url_ave', 'lug_ave', 'img_ave', "DATE_FORMAT(fec_ave,'%d') AS fecha", "concat(DAYNAME(fec_ave), ' ', MONTHNAME(fec_ave)   )  as dia", " DATE_FORMAT(hor_ave, '%l:%i %p ') as hora", 'ali_ave'];
        

        $arraywhere = ['cod_mne_ave' => $contenidoEvento[0]['cod_mne']];
        $eventos = $agendaeventos->select($array)->where($arraywhere)->orderBy('fec_ave ASC, hor_ave ASC')->find();
        // print_r($eventos);
