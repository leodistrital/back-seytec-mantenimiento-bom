...
class Guia extends  MyApiRest
{
    ...
    public function index($tipoguia = 'participantes')
    {
		...
		$bannersdata = $banners->select($array)->where('tip_ban', 4)->orderBy('ord_ban')->find();
        // print_r($bannersdata);



        return view('panelguia', ['contenido' => $data[0], 'seccion' => $seccion, 'tipoguia' => $tipoguia, 'filtros' => $filtros, 'formFiltros' => $formFiltros, 'totalRegstrosGlobal' => $this->totalRegstrosGlobal, 'resultadosporPagina' => $this->resultadosporPagina ,'bannersdata' =>$bannersdata]);
        // return '';
    }
    ...
    public function getCardsEmpresas($tipo, $formFiltros)
    {
        ...
        // }
         ...
         if (!empty($formFiltros['sector'])) {
            $usuarios->like('actividad', $formFiltros['sector'], 'match');
            // $usuariostotales->like('actividad', $formFiltros['textoform'], 'match');
        }
        ...
        // echo $usuariostotales->getLastQuery();
        // echo "+++++";
    ...
    public function getRepresentantes($empresa)
    {
        ...
        $wherearray = [3, 10];
        $arrayfiltroswhere = ['nombreempresa' => $empresa, 'excluir' => 0];
